...
func (uc *UserCreate) AddFormResponseIDs(ids ...int) *UserCreate {
	uc.mutation.AddFormResponseIDs(ids...)
	return uc
}

// AddFormResponses adds the "form_responses" edges to the Form_Response entity.
func (uc *UserCreate) AddFormResponses(f ...*Form_Response) *UserCreate {
	ids := make([]int, len(f))
	for i := range f {
		ids[i] = f[i].ID
	}
